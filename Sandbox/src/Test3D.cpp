...
void Test3D::OnInit(Renderer3D& renderer, Scene& scene)
{
	...
	Model* daggerModel = rdnew Model("/models/Dagger.spm", rdnew MaterialInstance(m_DaggerMaterial));
	...
	Model* cubeModel = rdnew Model("/models/RoundedCube.spm", rdnew MaterialInstance(cubeMaterial));
	m_Cube = rdnew Entity(cubeModel->GetMesh(), mat4::Rotate(90.0f, vec3(0, 0, 1)) * mat4::Translate(g_CubeTransform));
	m_Scene->Add(m_Cube);

	Model* sphereModel = rdnew Model("/models/Sphere.spm");
...
void Test3D::OnUpdate(const radi::Timestep& ts)
{
