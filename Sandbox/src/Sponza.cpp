...
void Sponza::OnInit(Renderer3D& renderer, Scene& scene)
{
	...
	for (int i = 0; i < models.size(); i++) {
		...
		Model* model = rdnew Model("res/Sponza/" + models[i] + "/model.spm", rdnew MaterialInstance(material));
		Entity* entity = rdnew Entity(model->GetMesh(), mat4::Translate(vec3(0, 0, 0))* mat4::Rotate(-90, vec3::XAxis()));
		m_Scene->Add(entity);
	}
	//Reset it
	...
	lights->Add(rdnew Light(vec3(0.4f)));
...
void Sponza::OnEvent(radi::events::Event& event)
{
