...
namespace radi {
	...
	#define RADI_MEMORY_ALIGNMENT 16
	#define RADI_ALLOC(size)	_aligned_malloc(size, RADI_MEMORY_ALIGNMENT)
	#define RADI_FREE(block)	_aligned_free(block);
	...
	void* Allocator::Allocate(size_t size)
	{
		...
		byte* result = (byte*)RADI_ALLOC(actualSize);
