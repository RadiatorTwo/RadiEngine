...
namespace radi {
	...
	void InputManager::PlatformInit()
	{
		...
		{
			RADI_ERROR("Failed registering raw input devices!");
		}
	...
	void InputManager::PlatformUpdateMessage()
	{
		...
		{
			if ((bool)(m_MouseClicked[RD_MOUSE_1] = raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_1_DOWN)) m_MouseButtons[RD_MOUSE_1] = true;
			else if ((bool)(raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_1_UP))
			{
				m_MouseButtons[RD_MOUSE_1] = false;
				m_EventCallback(MouseReleasedEvent(RD_MOUSE_1, m_MousePosition.x, m_MousePosition.y));
			}

			if ((bool)(m_MouseClicked[RD_MOUSE_2] = raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_2_DOWN)) m_MouseButtons[RD_MOUSE_2] = true;
			else if ((bool)(raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_2_UP))
			{
				m_MouseButtons[RD_MOUSE_2] = false;
				m_EventCallback(MouseReleasedEvent(RD_MOUSE_2, m_MousePosition.x, m_MousePosition.y));
			}

			if ((bool)(m_MouseClicked[RD_MOUSE_3] = raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_3_DOWN)) m_MouseButtons[RD_MOUSE_3] = true;
			else if ((bool)(raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_3_UP))
			{
				m_MouseButtons[RD_MOUSE_3] = false;
				m_EventCallback(MouseReleasedEvent(RD_MOUSE_3, m_MousePosition.x, m_MousePosition.y));
			}

			if ((bool)(m_MouseClicked[RD_MOUSE_4] = raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_4_DOWN)) m_MouseButtons[RD_MOUSE_4] = true;
			else if ((bool)(raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_4_UP))
			{
				m_MouseButtons[RD_MOUSE_4] = false;
				m_EventCallback(MouseReleasedEvent(RD_MOUSE_4, m_MousePosition.x, m_MousePosition.y));
			}

			if ((bool)(m_MouseClicked[RD_MOUSE_5] = raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_5_DOWN)) m_MouseButtons[RD_MOUSE_5] = true;
			else if ((bool)(raw->data.mouse.ulButtons & RI_MOUSE_BUTTON_5_UP))
			{
				m_MouseButtons[RD_MOUSE_5] = false;
				m_EventCallback(MouseReleasedEvent(RD_MOUSE_5, m_MousePosition.x, m_MousePosition.y));
			}

			for (int i = RD_MOUSE_1; i <= RD_MOUSE_5; i++)
			{
			...
			if (raw->data.mouse.lLastX != 0 || raw->data.mouse.lLastY != 0)
			{
				...
				m_EventCallback(MouseMovedEvent(m_MousePosition.x, m_MousePosition.y, m_MouseButtons[RD_MOUSE_1]));
			}

			if (raw->data.mouse.ulButtons & RI_MOUSE_WHEEL)
			{
				m_MouseWheelDeltaCurrent += raw->data.mouse.usButtonData == 120 ? 1 : -1;
				m_EventCallback(MousePressedEvent(raw->data.mouse.usButtonData == 120 ? RD_MWHEEL_UP : RD_MWHEEL_DOWN, m_MousePosition.x, m_MousePosition.y));
			}
		}
		...
		if (raw->header.dwType == RIM_TYPEKEYBOARD)
		{
			...
			{
				if (raw->data.keyboard.VKey == RD_KEY_CONTROL) raw->data.keyboard.VKey = RD_KEY_RCONTROL;
				else if (raw->data.keyboard.VKey == RD_KEY_MENU) raw->data.keyboard.VKey = RD_KEY_RMENU;
			}

			if (raw->data.keyboard.MakeCode == 54) raw->data.keyboard.VKey = RD_KEY_RSHIFT;
			...
			case RD_KEY_RCONTROL:
				modifier = RD_MODIFIER_RIGHT_CONTROL;
				break;
			case RD_KEY_RMENU:
				modifier = RD_MODIFIER_RIGHT_ALT;
				break;
			case RD_KEY_RSHIFT:
				modifier = RD_MODIFIER_RIGHT_SHIFT;
				break;
