	...
	namespace graphics {
		namespace API {
			...
			void D3DShader::Load(const String& source)
			{
				...
				RADI_ASSERT(m_Data.vs, info.profile, info.message);
				m_Data.ps = Compile(source, "ps_4_0", "PSMain", info);
				RADI_ASSERT(m_Data.ps, info.profile, info.message);
			...
			void D3DShader::ParseStruct(const String& block)
			{
				...
				ShaderStruct* shaderStruct = rdnew ShaderStruct(structName);
				index++; // {
				while (index < tokens.size())
				{
					...
					ShaderUniformDeclaration* field = rdnew D3DShaderUniformDeclaration(D3DShaderUniformDeclaration::StringToType(type), name);
				...
				while (index < tokens.size())
				{
					...
					if (buffer == nullptr)
					{
						...
						else
						{
							...
							{
							case 0:
								RADI_ASSERT(m_VSUserUniformBuffer == nullptr);
								m_VSUserUniformBuffer = buffer;
								break;
							case 1:
								RADI_ASSERT(m_PSUserUniformBuffer == nullptr);
			...
			void D3DShader::ParseTexture(const String& statement)
			{
				...
				D3DShaderResourceDeclaration* declaration = rdnew D3DShaderResourceDeclaration(D3DShaderResourceDeclaration::StringToType(type), name);
			...
			{
				std::vector<String> tokens = Tokenize(statement);
				RADI_ASSERT(tokens.front() == "SamplerState");
			...
			{
				if (m_VSUserUniformBuffer)
					RADI_ASSERT(slot != m_VSUserUniformBuffer->GetRegister());
			...
			{
				RADI_ASSERT(!m_PSUserUniformBuffer || slot != m_PSUserUniformBuffer->GetRegister());
