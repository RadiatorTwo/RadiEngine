...
#define RENDERER_MAX_TEXTURES	32 - 1
		...
		void Renderer2D::Init()
		{
			...
			uint* indices = rdnew uint[RENDERER_INDICES_SIZE];
			...
			m_PostEffects = rdnew PostEffects();
		...
		void Renderer2D::Begin()
		{
			if (m_Target == RenderTarget::BUFFER)
			{
				...
				if (m_ViewportSize != m_Framebuffer->GetSize())
				{
					...
					m_Framebuffer = rdnew API::Framebuffer2D(m_ViewportSize);

					if (m_PostEffectsEnabled)
					{
						delete m_PostEffectsBuffer;
						m_PostEffectsBuffer = rdnew API::Framebuffer2D(m_ViewportSize);
					}
				}
